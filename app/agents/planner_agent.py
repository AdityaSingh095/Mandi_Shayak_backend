    ...
    for idx, c_item in enumerate(crops_input):
        ...
        if db:
            try:
                ...
                from app.agents.price_fetch_agent import MandiInfo, _get_cached_prices, _fetch_from_agmarknet, _upsert_price_records
                ...
                # ── Fast State-Level Batch Pre-fetch (1 single HTTP call per crop) ──
                state_batch = []
                try:
                    state_batch = await _fetch_from_agmarknet(state=state, district=None, commodity=c_name, days_back=14)
                except Exception:
                    pass
                ...
                for m in db_mandis:
                    ...
                    if m_dist <= max_travel_radius_km:
                        ...
                        if cached and len(cached) > 0:
                            ...
                            # 2. Match from pre-fetched state batch
                            matched = [
                                r for r in state_batch
                                if r["market"].lower().strip() in m.name.lower().strip()
                                or m.name.lower().strip() in r["market"].lower().strip()
                            ]
                            if matched:
                                real_price = float(matched[-1]["modal_price"])
                                await _upsert_price_records(db, matched, m.id, canon_id)
                            else:
                                # 3. Distinct demo price fallback per mandi
                                demo_recs = get_demo_prices_for_mandi_crop(m.name, c_name, 14)
                                real_price = float(demo_recs[-1]["modal_price"])
